...
class ModuleObject extends MasterObject
{
    ...
    function Checkout_save()
    {
		...
		logic('notify')->Call($order['userid'], 'logic.order.MakeSure', array(
    ...
    function Order()
    {
		...
		if( $order['is_countdown'] == 1 ){
			$timeLimit_free = 5;			$order['timelimit'] = ($order['buytime'] + 60 * $timeLimit_free) - time();
					}
	...
	function Trust_pay()
	{
		$pay = logic('pay')->apiz($_REQUEST['payment_type']);
		$pay->__ActionPay($_REQUEST);
		//include handler('template')->file('buy_pay');
	}
