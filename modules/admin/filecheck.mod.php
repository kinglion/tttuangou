	...
	public function checkout()
	{
		...
		foreach ($showlist as $file => $md5)
		{
			...
			if (@array_key_exists($file, $modifylist))
			{
				...
				$dirlog[$dir]['modify']++;  				$dirlog[$dir]['marker'] = substr(md5($dir),0,3);
			}
			elseif (@array_key_exists($file, $dellist))
			{
				...
				$dirlog[$dir]['del']++;     				$dirlog[$dir]['marker'] = substr(md5($dir),0,3);
			}
			elseif (@array_key_exists($file, $addlist))
			{
				...
				$dirlog[$dir]['add']++;     				$dirlog[$dir]['marker'] = substr(md5($dir),0,3);
			}
			else
			{
