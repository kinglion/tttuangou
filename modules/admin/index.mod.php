...
class ModuleObject extends MasterObject
{
	...
	function _recommendList() {
        ...
        '/tttuangou.net/" target="_blank">地方网站创业：用志扬互动系统</a></li><li class="liS"><a href="http:/'.'/biniu.com" target="_blank">比牛，让业务牛逼起来（微信营销平台）</a></li>'
		);
	}
	...
	function home() 
	{
		...
		$order_nums  = dbc(DBCMax)->select('order')->
			in('count(*) as NUM')->where("buytime between {$today_start} and {$today_end} or paytime between {$today_start} and {$today_end}")->limit(1)->done();
		$pay_ordes   = dbc(DBCMax)->select('order')->
			in('count(*) as NUM,sum(paymoney) as money')->where("paytime between {$today_start} and {$today_end} and paytime > 0 and pay > 0")->limit(1)->done();
		$statistic['today_orders'] = $order_nums['NUM'];
		$statistic['today_pay_orders'] = $pay_ordes['NUM'];
		$statistic['today_unpay_orders'] = $order_nums['NUM'] - $pay_ordes['NUM'];
