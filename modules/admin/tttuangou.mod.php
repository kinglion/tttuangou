...
class ModuleObject extends MasterObject{
	...
	function Doaddproduct(){
		...
		if(!(is_numeric($price) && is_numeric($nowprice) && $price>$nowprice) ){
			$this->Messager("价格必须是数字，而且原始价格必须大于" . TUANGOU_STR . "价格。", -1);
		};
		$begintime = strtotime($begintime);
		$overtime = strtotime($overtime);
		if($begintime>$overtime) $this->Messager(TUANGOU_STR . "开始时间必须小于结束时间哦！", -1);
		$somewrong = '';
				$img = '';
		...
		if($product['status']==0){
			...
			if($order==''){
				...
				foreach($order as $i => $value){
					...
					'title'=>TUANGOU_STR . '失败提示信息',
					'content'=>'非常抱歉！您' . TUANGOU_STR . '的产品（'.$product['name'].'）因为购买人数不足而失败了！货款已经返还您的账户请点<a href="'.$this->Config['site_url'].'">这里</a>查看详细！',
					'addtime'=>time()
					);
					$keys=$values='';
					...
					$this->ProductLogic->delSellerTotMoney($value['sellerid'],$price);
						...
						'userid' => $value['userid'],
						'type' => 1,
						'name' => '失败' . TUANGOU_STR . '返款账户',
						'intro' => '返还您' . TUANGOU_STR . '金额'.$price.'元',
						'money' => $price,
						'time' => time(),
					);
	...
	function Mailcallpay(){
		...
		'title'=>'订单支付温馨提示',
		'content'=>'温馨提示您' . TUANGOU_STR . '的产品（'.$order['name'].')即将结束,而您还没有付款，想拥有现在是好机会啊，请点<a href="'.$this->Config['site_url'].'">这里完成付款立刻拥有该商品</a>！',
		'addtime'=>time()
		);
		$keys=$values='';
		foreach($ary as $i => $valuez){
	...
	function Doconfirmorder(){
		...
		if($product['id'] == '')
		{
			$this->Messager("错误~该产品不存在或已结束" . TUANGOU_STR . "!");
		};
		if($product['maxnum']!=0){
	...
	function Ticket($orderid,$userID){				$sql='SELECT p.name,p.type,p.nowprice,o.productid,p.id,p.city,p.successnum,o.orderid,o.userid as ouid,o.productnum FROM '.TABLE_PREFIX.'tttuangou_order o left join '.TABLE_PREFIX.'tttuangou_product p on o.productid = p.id where  o.orderid = '.$orderid;
		...
		if($num<$order['successnum']){
			$this->Messager('您已经成功' . TUANGOU_STR . '该产品，但' . TUANGOU_STR . '差'.($order['successnum']-$num).'人达成 ，快去邀请好友参加团购，还能获得丰厚获取奖励!','?mod=tttuangou&code=listorder');
		}
		if($num==$order['successnum'])
		{
			...
			foreach($orderPayed as $i => $value){
				...
				'address'=> $value['email'],
				'username'=>$value['username'],
				'title'=>TUANGOU_STR . '成功提示信息',
				'content'=>'恭喜您在本站' . TUANGOU_STR . '的产品('.$order['name'].')在'.date('Y-m-d H:i',time()). TUANGOU_STR . '成功，请点<a href="'.$this->Config['site_url'].'">这里</a>查看您的团购券！',
				'addtime'=>time()
				);
			...
			$this->Messager(TUANGOU_STR . "成功，您还可以推荐您的朋友来购买哦！",'?mod=tttuangou&code=listorder');
		};
		$this->ProductLogic->productTypedit($order['productid'],2);
	...
	function Ticketz(){
		...
		$num=$query->GetRow();
		$num=$num['count(*)'];
		if($addsql!='' && $num==0)$this->Messager("符合该条件的" . TUANGOU_STR . "券找不到",'?mod=tttuangou&code=ticket');
