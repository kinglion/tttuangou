	...
	function HasPermission($mod,$act,$is_admin=0) 
	{
		...
		if((($current_action_id=$this->ActionList[$mod]['index'][$action])!==null) || (($current_action_id=$this->ActionList[$mod]['index']["*"])!==null))
		{
			...
			if($current_action['allow_all']==1)Return true; 
						...
						$role_privilege = explode(',', $role_privilege);
			if(in_array($current_action_id, $role_privilege)===false) 
			{
				$this->_SetError("您的角色({$role_name})没有{$current_action['name']}权限");
				return false;
			}
		}
		else
		{
			...
			if(!$is_admin) return true; 			
						if(X_IS_AJAX && $mod = 'catalog' && $action = 'subclass') return true; 
			if($mod == 'tttuangou' && $action == 'addmap') return true;
			if((int) $this->Config['aijuhe_founder'] < 1) return true; 			if('POST' != $_SERVER['REQUEST_METHOD']) return true; 			$this->_SetError("您的角色({$role_name})没有{$mod}/{$action}权限");
			return false;
		}
