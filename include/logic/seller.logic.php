...
class SellerLogic
{
	...
	public function Add($city, $userid, $sellername = '合作商家', $sellerphone = '', $selleraddress = '', $sellerurl = '', $map = '', $rebate)
	{
		extract($rebate);
		$data = array(
	...
	public function Join($in){
		$fields = array('userid','area','sellername','selleraddress','sellerphone','sellerurl','sellermap','profit_id');
		foreach($fields as $f){
			$data[$f] = $in[$f];
		}
		...
		$data['time'] = time();
		dbc(DBCMax)->update('members')->data('role_id=6')->where('uid='.(int)$data['userid'])->done();
		dbc()->SetTable(table('seller'));
		return dbc()->Insert($data);
	}
