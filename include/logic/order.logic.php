...
class OrderLogic
{
	...
	public function FreeCountDownOrder()
	{
		$timeLimit_free = 7;		$timelimit = time() - (60 * $timeLimit_free);
						$sql_where['is_countdown'] = 1;
		$sql_where['buytime'] < $timelimit;
						$sql_where_process =  ' AND process<>"_TimeLimit_" AND (process="__CREATE__" OR process="WAIT_BUYER_PAY")';
	...
	{
				logic('seller')->money_add($order['product']['sellerid'], ($order['totalprice']-$order['expressprice']));		logic('seller')->order_success($order['product']['sellerid']);
