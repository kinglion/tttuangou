...
class RebateLogic
{
	...
	public function get_my_rebate_list(){
		$sql = "SELECT r.*,t.number FROM ". table('rebate_log') ." r LEFT JOIN ". table('ticket') ." t ON r.ticketid = t.ticketid WHERE r.uid='". MEMBER_ID ."' AND r.type='master' ORDER BY r.id DESC";
		$sql = page_moyo($sql);
		$res = dbc(DBCMax)->query($sql)->done();
		if( $res ){
			return $res;
