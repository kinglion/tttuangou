...
</font> <ul class="navs"> <li><a id="nav2Base" href="#p_base">1.基本信息</a></li> <li><a id="nav2Intro" href="#p_intro">2.详情介绍</a></li> <li><a id="nav2Image" href="#p_image">3.产品图片</a></li> <li><a id="nav2Type" href="#p_type">4.产品设置</a></li> <li><a id="nav2Extend" href="#p_extend">5.其他设置</a></li> <li><a id="nav2Attrs" href="#p_attrs">6.属性设置</a></li> </ul> <font id="autoSaveStatus" style="float: right;margin-top: 7px;margin-right: 40px;_margin-right: 10px;"></font> </div> <div class="items"> <div id="p_base"> <table width="100%" class="tableborder"> <tr> <td width="100" class="td_title">产品标题：</td> <td> <input id="productName" name="name" type="text" size="80" value="<?=$p['name']?>" />
（吸引眼球第一要素）
...
<table width="100%" class="tableborder" > <tr> <td width="100" class="td_title">投放城市：</td> <td> <div id="fillIfoCity"> <select name="city" id="allCityList"> <option value="-1">正在加载</option> </select>
...
</div> <script type="text/javascript">var __Default_CityID = "<?=$p['city']?>";var __Default_SellerID = "<?=$p['sellerid']?>";</script> </td> </tr> <tr> <td width="100" class="td_title">显示方式：</td> <td> <div id="fillIfoDisplay"> <select name="display"> <option value="<?=PRO_DSP_None?>">不在前台显示</option> <option value="<?=PRO_DSP_City?>">在指定城市显示</option> <option value="<?=PRO_DSP_Global?>"   selected="selected">在全部城市显示</option> </select> </div> </td> </tr> <tr> <td width="100" class="td_title">显示优先级：</td> <td> <input name="order" type="text" size="3" value="<? echo (int)$p['order']; ?>" />&nbsp;&nbsp;（数字越大，显示位置越靠前，用于同时<?=TUANGOU_STR?>多个产品的时候）
...
</td> </tr> <tr> <td width="160" class="td_title"> <font id="price_dsp_presell" <? echo $p['presell'] ? '' : 'style="display:none;"'; ?>>预付价：</font> <font id="price_dsp_normal" <? echo $p['presell'] ? 'style="display:none;"' : ''; ?>><?=TUANGOU_STR?>价：</font> </td> <td> <input id="productNowPrice" name="nowprice" type="text" size="6" value="<? echo (float)$p['nowprice']; ?>" />&nbsp;&nbsp;元（如果启用下面的预付模式，此处将变更为预付价）
</td> </tr> <tr style="display:none;"> <td width="160" class="td_title">商家结算价：</td> <td> <input id="FundPrice" name="fundprice" type="text" size="6" value="<? echo $p['fundprice']<0 ? '' : (float)$p['fundprice']; ?>" />&nbsp;&nbsp;元（如果设置了该价格，则按照该价格与商家进行结算；比如设置5.25元，则每单分给商家5.25元；）<br>
（注意：【20140515更新】如果填写0则代表不给商家分成；如果留空，则按照该商家与站长的分成比例与商家进行结算。(商家与站长的分成比例可通过“商家管理>>修改商家”进行设置)）
</td> </tr> <tr style="display:none;"> <td width="160" class="td_title">赠送积分：</td> <td> <input id="Score" name="score" type="text" size="6" value="<?=$p['score']?>" />&nbsp;&nbsp;分（如果设置了该分数值，则买家在成功购买了该产品后，会获得相应积分）
</td> </tr> <tr  style="display:none;"> <td width="160" class="td_title">
...
<br> <label>预付价的显示名称：<input id="presellText" name="presell_text" type="text" size="6" value="<? echo $p['presell']['text']; ?>" />（比如预付或在线支付）</label><br> <label><?=TUANGOU_STR?>价：<input id="presellPrice" name="presell_price" type="text" size="6" value="<? echo $p['presell']['price_full']; ?>" />&nbsp;&nbsp;元（启用预付模式后，需在此填写一个<?=TUANGOU_STR?>价）</label> </td> </tr> <script type="text/javascript">
...
</td> </tr> <tr style="display:none;"> <td width="160" class="td_title">限时购模式：</td> <td> <input type="checkbox" name="is_countdown" value="1" 
<? if($p['is_countdown']) { ?>
checked="checked"
<? } ?>
/>
...
<? } ?>
</td> </tr> <tr style="display:none;"> <td width="160" class="td_title"><?=TUANGOU_STR?>类型：</td> <td> <select name="type" onchange="product_type_show(this)" id="product_type_sel">
<? if($p['type']!='prize') { ?>
<option value="ticket"
<? if($p['type']=='ticket') { ?>
 selected="selected"
...
</script> <tr class="displayer_of_ticket" style="display:none;"> <td width="160" class="td_title"><?=TUANGOU_STR?>券有效期至：</td> <td> <input type="text" onfocus="WdatePicker({isShowClear:false,readOnly:true,dateFmt:'yyyy年MM月dd日 HH时mm分ss秒',startDate:'<? echo date('Y-m-d H:i:s', $p['perioddate'] ? $p['perioddate'] : time()+604800);; ?>',vel:'perioddate'})" size="35" class="Wdate" value="<? echo date('Y年m月d日 H时i分s秒', $p['perioddate'] ? $p['perioddate'] : time()+604800);; ?>" /> <input name="perioddate" type="hidden" id="perioddate" value="<? echo date('Y-m-d H:i:s', $p['perioddate'] ? $p['perioddate'] : time()+604800);; ?>" /> </td> </tr> <tr class="displayer_of_ticket" style="display:none;"> <td width="160" class="td_title">多券合一：</td> <td> <label><input name="allinone" type="radio" value="true"
<? if($p['allinone']=='true') { ?>
 checked="checked"
<? } ?>
...
</td> </tr> </table> <table width="100%" class="tableborder"> <tr> <td width="160" class="td_title">成功<?=TUANGOU_STR?>人数：</td> <td> <input name="successnum" type="text" id="successnum" value="<? echo (int)$p['successnum']; ?>" />
&nbsp;&nbsp;最少需要多少人购买才算<?=TUANGOU_STR?>成功。注意：如是<?=TUANGOU_STR?>券，那只有达到<?=TUANGOU_STR?>人数才会生成
...
</td> </tr> <tr style="display:none;"> <td width="160" class="td_title">是否参与邀请返利：</td> <td> <label><input name="irebates" type="radio" value="true"
<? if(meta('p_ir_'.$p['id'])) { ?>
 checked="checked"
<? } ?>
/> 是</label>
...
<? if(!meta('p_ir_'.$p['id'])) { ?>
 checked="checked"
<? } ?>
/> 否</label> </td> </tr> </table> <table width="100%" class="tableborder" style="display:none;"> <tr> <td width="160" class="td_title">支付设置：</td> <td> <label><input name="specialPayment" type="radio" value="false"
...
</div> </td> </tr> </table> <table width="100%" class="tableborder" style="display:none;"> <tr> <td width="160" class="td_title">通知方式：</td> <td>
<? $ntc = meta('p_nt_'.$p['id']) ?>
...
</script> </div> </div> </div> <div class="submitArea"> <input id="submitButton" type="submit" class="button back2" value="保存" /> </div> </form>
