...
<? } ?>
<a href="?mod=ui&code=igos&op=config">多团展示方式设置</a> </div> <table id="productTable" cellspacing="1" cellpadding="4" width="100%" align="center" class="tableborder"> <thead> <tr class="tr_nav"> <td width="22%">产品名称（悬浮查看长标题）</td> <td width="12%">销售概览</td> <td width="13%">金额统计</td> <td width="13%">运行状态</td> <td width="12%">显示设定</td> <td width="12%">订阅通知</td> <td width="12%">管理操作</td> </tr> </thead> <tbody> 
<? if(is_array($list)) { foreach($list as $one) { ?>
 <tr> <td title="<?=$one['name']?>"> <a href="<?=$this->Config['site_url']?>/?view=<?=$one['id']?>" target="_blank"><?=$one['flag']?></a> </td> <td>
...
退款：￥<?=$one['mny_refund']?>
</td> <td><? echo logic('product')->STA_Name($one['status']); ?>
...
<? } } ?>
 </tbody> <tfoot> <tr> <td colspan="7"><?=page_moyo()?></td> </tr> </tfoot> </table>
